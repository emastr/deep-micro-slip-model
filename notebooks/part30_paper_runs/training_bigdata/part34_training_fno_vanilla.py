...
#data_dir = "/mnt/data0/emastr/geometry_torch/variance_norepar_512/"
#save_dir = "/mnt/data0/emastr/article_training_nodecay/"
save_dir = "/mnt/data0/emastr/training/article_training_hugedata_epochlr_bigbatch/"
...
for seed in [0, 1, 2, 3, 4, 5]:
    ...
    session = Session(net, save_name=save_name, save_dir=save_dir, dash_dir=dash_dir + "v4", device=device, path_data=data_dir, lr_schedule=True)
    session.train_nsteps(N_train)
    
    print(f"Seed {seed}, model ver2")
    save_name = f"fno_vanilla_ver2_seed{seed}"
    net = fno_ver2(device=device) 
    session = Session(net, save_name=save_name, save_dir=save_dir, dash_dir=dash_dir + "v2", device=device, path_data=data_dir, lr_schedule=True)
    session.train_nsteps(N_train)
    #
    print(f"Seed {seed}, model ver3")    
    save_name = f"fno_vanilla_ver3_seed{seed}"
    net = fno_ver3(device=device) 
    session = Session(net, save_name=save_name, save_dir=save_dir, dash_dir=dash_dir + "v3", device=device, path_data=data_dir, lr_schedule=True)
    session.train_nsteps(N_train)
    
    print(f"Seed {seed}, model ver1")
    save_name = f"fno_vanilla_ver1_seed{seed}"
    net = fno_ver1(device=device) 
    session = Session(net, save_name=save_name, save_dir=save_dir, dash_dir=dash_dir + "v1", device=device, path_data=data_dir, lr_schedule=True)
    session.train_nsteps(N_train)
