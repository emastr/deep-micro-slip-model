...
def solveNavierStokes_fromBasis(u0, v0, f, g, alpha, uTop, beta, vTop, vBot, dom, xDim, yDim, **kwargs):
    callback = kwargs.pop("callback", lambda *args: None)
    T = kwargs.pop("T", 1.)
    N = kwargs.pop("N", 100)
    dt = T/N
    ...
    for n in range(N-1):
        ...
        f_n = f + (u * dx_u + v * dy_u)  - (1/ dt) * u 
        g_n = g + (u * dx_v + v * dy_v)  - (1/ dt) * v
        
        u, v, info = solveNavierTimeStep_fromBasis(f_n, g_n, alpha, uTop, beta, vTop, vBot, -1/dt, dom, xDim, yDim, **kwargs)
        callback(n, u, v, info)
    return u, v
    


def solveNavierTimeStep_fromBasis(f, g, alpha, uTop, beta, vTop, vBot, lam, dom, xDim, yDim, **kwargs):
    """Solve from Basis Function objects"""
    uC,vC,info = solveNavierTimeStep(f.basis.coef, g.basis.coef, alpha.basis.coef, uTop.basis.coef, beta.basis.coef, vTop.basis.coef, vBot.basis.coef, lam, dom, xDim, yDim, **kwargs)
...
def solveNavierTimeStep(f, g, alpha, uTop, beta, vTop, vBot, lam, dom,  Nx, Ny, with_gmres=False, **gmresargs):
    """SOLVEROBINSTOKES   Solve 1-periodic Stokes equations on 2D domain [0,Lx]x[0, Ly].
     ...
     solveRobinStokes then solves The following boundary value problem:
         ...
         PDE: Moment equations
           lam * u + (DxDx + DyDy)*u - Dx*p = f on [0,Lx]x[0,Ly]
           lam * v + (DxDx + DyDy)*v - Dy*p = g on [0,Lx]x[0,Ly]
    ...
    B11 = kron(lam * Idx[:kmax, :] + Dx2[:kmax,:], iDy2[1:, :])+kron(Idx[:kmax,:], Idy3)
    B12 = -(kron(Dx3[:kmax,:], iDy3)+kron(Dx[:kmax,:], iDy[2:, :] + lam * iDy3))
    B21 = kron(unit_const, Idy2 + lam * iDy2)
    B22 = zero_const
    B31 = kron(lam * Idx[kmax+1:, :] + Dx2[kmax+1:,:], iDy2[1:, :])+kron(Idx[kmax+1:,:], Idy3)
    B32 = -(kron(Dx3[kmax+1:,:], iDy3)+kron(Dx[kmax+1:,:], iDy[2:, :] + lam * iDy3))
    B41 = kron(Dx, iDy)
    B42 = kron(Idx, Idy)
    ...
    if not with_gmres:
        ...
        uv, _ = gmres(B, b, tol=1e-5, maxiter=np.sqrt(B.shape[0]),  **gmresargs)
...
def testSolve(xDim, yDim, N, T=1., plot=True, **kwargs):
    ...
    u0, v0, info = solveNavierTimeStep_fromBasis(
                                  f_basis, 
                                  g_basis, 
                                  alpha_basis, 
                                  uTop_basis, 
    ...
    uSol_basis, vSol_basis = solveNavierStokes_fromBasis(
                                    u0,
                                    v0,
                                    f_basis,
                                    g_basis,
...
def testSolve2(xDim, yDim, N, T=1., plot=True, **kwargs):
    import matplotlib.pyplot as plt
    ...
    uTop = lambda x: np.ones_like(x) + 1. * np.sin(x / Lx * k * 2*np.pi)
    vTop = lambda x: np.zeros_like(x)
    
    uBot = lambda x: -(np.zeros_like(x) + 1. * np.sin(x / Lx * k * 2*np.pi))
    vBot = lambda x: np.zeros_like(x)
    
    alpha = lambda x: np.zeros_like(x)
    beta = uBot
    
    # F, G
    f_basis = ScaleShiftedBasisProduct.fromFunctionInDomain(f, dom, xDim, yDim, xBasis, yBasis)
    g_basis = ScaleShiftedBasisProduct.fromFunctionInDomain(g, dom, xDim, yDim, xBasis, yBasis)
    
    # Upper boundary condition
    uTop_basis = ScaleShiftedBasis.fromFunctionInDomain(uTop, dom[0], xBasis, xDim)
    vTop_basis = ScaleShiftedBasis.fromFunctionInDomain(vTop, dom[0], xBasis, xDim)


    # Lower boundary velocities
    uBot_basis = ScaleShiftedBasis.fromFunctionInDomain(uBot, dom[0], xBasis, xDim)
    vBot_basis = ScaleShiftedBasis.fromFunctionInDomain(vBot, dom[0], xBasis, xDim)


    # Lower boundary slip coefficient
    beta_basis = ScaleShiftedBasis.fromFunctionInDomain(beta, dom[0], xBasis, xDim)
    alpha_basis = ScaleShiftedBasis.fromFunctionInDomain(alpha, dom[0], xBasis, xDim)
    ...
    if plot:
        ...
        plt.figure(figsize=(15,20))
        plt.subplot(321)
        plt.title("Init U component")
        u0.plot(plt.gca())
        plt.subplot(322)
        plt.title("Init V component")
        v0.plot(plt.gca())
        plt.subplot(323)
        plt.title("Solver U component")
        uSol_basis.plot(plt.gca())
        ...
        plt.title("Solver V component")
        vSol_basis.plot(plt.gca())
    return uSol_basis, vSol_basis, u0, v0
