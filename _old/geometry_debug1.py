...
class Geometry():
    ...
    def line_eval_adjoint(self, a, b, derivative=0, **kwargs):
        ...
        if derivative==0:
            #f = (-da * im(dg * log_ba) + im(c(da) * dg) * c(log_ba) + im(ga * c(da)) * c(dg) / c(ga_gb)) / (1j * np.pi)
            term1 = 1/np.pi * dg  * log(np.abs(ga) / np.abs(gb))
            term2 = -1/np.pi * c(dg) * (da/c(da)) * log(c(ga / gb))
            term3 = 1/np.pi * 1j * c(dg) * im(ga * c(gb)) / c(ga) / c(gb)
            f = (term1 + term2 + term3)
            #f = -np.abs(dg)/(np.pi * c(da)) * ( 2*np.log(np.abs(ga / gb)) * re(c(da) * e) \
             #                               + c(e) * im(c(da) * ga) * c(da) / (c(ga) * c(gb)))
            ...
            #for i in range(len(f)):
                #int1 = lambda t: -(lder) / (1j * np.pi)* im(dg[i] / (g[i] - lpos(t))) 
                #int2 = lambda t: -c(lder) / (1j * np.pi)* im(c(g[i] - lpos(t))*dg[i]) / (c(g[i] - lpos(t))**2)
                
                #int1 = lambda t: dg[i] / np.pi * re(lder / (g[i] - lpos(t))) 
                ...
                #ter1i = lambda t: dg[i] / np.pi * re(lder / (g[i] - lpos(t))) 
                #ter2i = lambda t: c(dg[i]) /np.pi * lder / c(lpos(t) - g[i])
                ...
                #f2[i] = lgrid.integrate(lambda t: ter1i(t) + ter2i(t) + ter3i(t))#int1(t) + int2(t))
                
            # 
            #idx = 100
            #print(f"{f2[idx]} vs analytic {f[idx]}")
            #print(f"term 1: {term1[idx]} vs analytic {ter1[idx]}, delta: {term1[idx]-ter1[idx]}")
            #print(f"term 2: {term2[idx]} vs analytic {ter2[idx]}, delta: {term2[idx]-ter2[idx]}")
            #print(f"term 3: {term3[idx]} vs analytic {ter3[idx]}, delta: {term3[idx]-ter3[idx]}")
            #print(f"delta: {term1[idx]-ter1[idx]}")
            #print(f"delta: {term2[idx]-ter2[idx]}")
            #print(f"delta: {term3[idx]-ter3[idx]}")
            #print(f2)
            #plt.plot(t2, np.imag(f2), label="im num")
            #plt.plot(t2, np.imag(f), '--', label="im anal")
            #plt.plot(t2, np.real(f2), label="re num")
            #plt.plot(t2, np.real(f), '--', label="re anal")
            #plt.legend()
            #plt.figure()
            #f =f2
